...
$apiUrl = 'http://localhost/hoa_system/app/api/users/getById.full-boardmembers.php?id=' . $id;
...
if (!$response || !$response['success'] || empty($response['data'])) {
    die('Error: Invalid API response or no data available');
}
...
$pdf->Output('D', 'user_' . $id . '_report.pdf');
exit;
