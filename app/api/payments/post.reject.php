...
try {
    // Fetch homeowner fee details using the reference number
    ...
    // Fetch fee assignment IDs that are related to the homeowner fee
    $feeSql = "
        SELECT fa.id
    ...
    $updateHfSql = "UPDATE homeowner_fees SET status = 0 WHERE ref_no = ?";
    $updateHfStmt = $conn->prepare($updateHfSql);
    $updateHfStmt->bind_param("s", $ref_no);
    $updateHfStmt->execute();
    $updateHfStmt->close();

    $updatePvSql = "UPDATE payment_verification SET status = 3 WHERE ref_no = ?";
    ...
    if (!empty($fee_ids)) {
        ...
        $updateFeeSql = "UPDATE fee_assignments SET status = 0 WHERE id IN ($placeholders)";
