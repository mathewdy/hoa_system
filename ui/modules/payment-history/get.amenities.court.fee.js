...
if (!$('[data-module="court-history"]').length) {
  console.log('[Court History] Not active on this page');
} else {
  console.log('[Court History] LOADED');
}

const BASE_URL = '/hoa_system/';
const API_URL = `${BASE_URL}app/api/payment-history/get.court.fee.php`;
...
const columns = [
  ...
  row => {
    ...
    if(role == 1 || role == 3 || role == 4){
      return `<a href="../../app/api/payment-history/getById.court.file.php?id=${row.id}" 
          target="_blank"
...
new TableView($state, fetcher, {
  tableId: 'courtHistoryTable',
  searchId: 'court-fee-search',
