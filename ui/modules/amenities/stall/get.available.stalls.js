...
if (!$('[data-module="stallrentals"]').length) {
  console.log('[Stalls] Not active on this page');
} else {
  console.log('[Stalls] LOADED');
}

const BASE_URL = '/hoa_system/';
const API_URL = `${BASE_URL}app/api/amenities/stall/get.all-stalls.php`;
...
const columns = [
  row => `<div class="text-gray-500">Stall ${row.stall_no ?? 0 }</div>`,
  row => row.status === 1
    ? '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Active</span>'
    : '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">Inactive</span>',
  row => {
    const created = new Date(row.date_created).toLocaleDateString('en-PH', {
      month: 'short',
      day: 'numeric',
      year: 'numeric'
    });

    return `<span class="text-gray-500">${created}</span>`;
  },
  row => { 
    return `<a 
