$(document).ready(() => {
  const apiUrl = `/hoa_system/app/api/users/get.boardmembers.php`;
  ...
  const columns = [
    u => {
      ...
      u => {
        ...
        return role !== '-' 
          ? `<span class="px-2.5 py-1.5 rounded-md text-xs font-medium border border-teal-300 text-teal-600">
               ${escape(role)}
             </span>`
          : `<span class="text-gray-400">${escape(role)}</span>`;
      },
