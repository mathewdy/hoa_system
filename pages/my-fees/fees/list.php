    ...
    <form class="flex flex-1 w-full">
      <div class="relative w-full">
        ...
        <input type="text" id="fee-search" 
          ...
          placeholder="Search fees (e.g. Monthly Dues)..." />
      </div>
    </form>
  </div>

  <div class="relative overflow-x-auto shadow-md sm:rounded-lg border">
    <table id="myFeesTable" class="w-full text-sm text-left text-gray-500">
    ...
    <nav class="flex items-center justify-between p-4 text-sm">
      <span class="text-gray-500">
        Showing <span id="rangeStart">1</span>-<span id="rangeEnd">15</span>
        of <span id="totalRecords">0</span>
      </span>
      <ul id="feesPagination" class="inline-flex -space-x-px h-8"></ul>
    </nav>
  </div>

  <!-- MODULE TRIGGER -->
  <div data-module="my-fees"></div>
</div>
...
<div id="paymentModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
  <div class="bg-white rounded-lg w-full max-w-2xl shadow-2xl flex flex-col max-h-screen">
    ...
    <div class="border-b border-gray-200 px-6 py-4">
      <h2 class="text-2xl font-semibold text-gray-900">Record Payment</h2>
      <p class="text-sm text-gray-600 mt-1">Submit your payment for selected fees</p>
    </div>
    ...
    <div class="border-t border-gray-200 px-6 py-4 bg-gray-50 rounded-b-lg flex justify-end gap-3">
      <button type="button" id="closePaymentModal" class="px-6 py-2.5 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition">Cancel</button>
      <button type="submit" form="payment-form" class="px-8 py-2.5 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition">Submit Payment</button>
    </div>
  </div>
</div>
...
$pageScripts = '
  <script type="module" src="/hoa_system/ui/modules/fees/get.user.fees.js"></script>
';
