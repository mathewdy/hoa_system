        ...
        if(mysqli_num_rows($run_sql_fetch_fee_type) > 0){
           foreach($run_sql_fetch_fee_type as $row_fee_type){
            ?>
                ...
                <form action="../../Query/approve-fee-type.php" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="fee_type_id" value="<?php echo $row_fee_type['fee_type_id']; ?>">
                <label for="">Fee Name:</label>
                <input type="text" name="fee_name" value="<?php echo $row_fee_type['fee_name']; ?>" readonly>
                <label for="">Description:</label>
                <textarea name="description" readonly><?php echo $row_fee_type['description']; ?></textarea>
                <label for="">Amount:</label>
                <input type="number" name="amount" value="<?php echo $row_fee_type['amount']; ?>" readonly>
                ...
                <div>
                    ...
                    ?>" readonly>
                    <select name="cadence" id="fee-cadence" disabled>
                ...
                <div>
                    ...
                    <input type="date" id="fee-start-date" value="<?php echo $row_fee_type['start_date'];?>" name="start_date" readonly>
                </div>
