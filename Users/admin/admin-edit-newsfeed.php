...
if (isset($_POST['update_post'])) {
    ...
    if (mysqli_query($conn, $updateSql)) {
        $success = "Post updated successfully!" . "<script>window.location.href='admin-newsfeed.php' </script>";
        // Refresh post data
        $sql = "SELECT * FROM news_feed WHERE id = $post_id";
        $result = mysqli_query($conn, $sql);
        $post = mysqli_fetch_assoc($result);
...
<div class="max-w-3xl mx-auto bg-white shadow rounded-lg p-6">
    ...
    <form action="" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="flex justify-end space-x-2">
            <a href="admin-newsfeed.php" class="py-2 px-4 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">Cancel</a>
